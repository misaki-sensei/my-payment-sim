...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const predictedBalanceFooter = document.getElementById('predictedBalanceFooter'); // このIDはHTMLに存在しないようです
    ...
    const processPayment = (amount, shopId, transactionId) => {
        if (currentBalance >= amount) {
            ...
            paymentStatusesRef.child(transactionId).set({
                ...
                alert("支払いステータスの更新中にエラーが発生しました。"); // エラー時にユーザーに通知
            });
    ...
    const startCamera = async () => {
        try {
            ...
            console.error("CUSTOMER: Camera access error:", err);
    ...
    const tick = () => {
        if (qrCameraVideo.readyState === qrCameraVideo.HAVE_ENOUGH_DATA) {
            ...
            if (code) {
                // QRコードを検出した場合
                console.log("CUSTOMER: QR Code detected. Raw data:", code.data); // QRコードの生データをログ出力
                try {
                    const qrData = JSON.parse(code.data);
                    console.log("CUSTOMER: Parsed QR Data:", qrData); // 解析されたQRデータをログ出力

                    if (qrData.amount && qrData.shopId && qrData.transactionId) { // transactionId もチェック
                        console.log("CUSTOMER: QR Data contains all required fields.");
                        ...
                        console.log(`CUSTOMER: Scanned: Amount=${scannedPaymentAmount}, ShopID=${scannedShopId}, TransactionID=${scannedTransactionId}`);
                        ...
                        cameraStatus.textContent = '不正なQRコード形式です。必要な情報が見つかりません。';
                        confirmPayBtn.disabled = true;
                    }
                } catch (e) {
                    console.error("CUSTOMER: Error parsing QR Code data as JSON:", e, "Raw data:", code.data);
                    cameraStatus.textContent = 'QRコードの内容を解析できませんでした。形式が正しくありません。';
                    confirmPayBtn.disabled = true;
                }
            } else {
                // QRコードを検出しない場合
    ...
    const updatePredictedBalance = () => {
        ...
        if (!isNaN(amountToCharge) && amountToCharge > 0) {
            ...
            predictedBalanceFooter.classList.remove('hidden'); // HTMLにpredictedBalanceFooterがない場合、エラーになります
        } else {
            predictedBalanceDisplay.textContent = `¥ ${currentBalance.toLocaleString()}`; // 有効でない場合は現在の残高を表示
            predictedBalanceFooter.classList.add('hidden'); // HTMLにpredictedBalanceFooterがない場合、エラーになります
        }
    };
